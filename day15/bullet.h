...
typedef struct _S_BULLET_OBJECT {
	...
	double m_fCenterX; //애니메이션의 중심점. 자기자신이 움직일때 중심.
	double m_fCenterY;
	...
	double m_faccLifeTime; //현재 생존 시간
	...
	void (*pfFire)(struct _S_BULLET_OBJECT *pObj, int x, int y, double speed, double vx, double vy, double lifeLimit);
