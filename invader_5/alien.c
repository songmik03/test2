...
static void Apply(_S_ALIEN_OBJECT *pObj,double deltaTick,double randNum)
{
	...
	switch(pObj->m_nFSM) {
		...
		if(pObj->m_pBullet != NULL) {
			if(pObj->m_pBullet->m_nFSM ==0) {
				pObj->m_pBullet->pfFire(pObj->m_pBullet,pObj->m_fXpos+2,pObj->m_fYpos,5.0 +randNum,0,1.0,5.0);
			}
		}
		break;
